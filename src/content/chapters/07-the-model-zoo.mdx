---
title: "The model zoo"
blurb: "Trees, forests, boosting and nearest neighbours — what each one quietly assumes, and why on table-shaped data they still beat neural networks."
order: 7
day: 7
part: foundations
minutes: 90
...
<Py id="fit-predict">`fit` and `predict`</Py>. Every model in the scikit-learn
library answers to those two words and nothing else: `fit` to learn from
examples, `predict` to answer a new question. Which is why trying five completely
different kinds of model is a loop rather than a project.
