...
blurb: "From guessing the next word to answering a question: tokens, scaling laws, the three stages of training, and what the temperature setting does."
order: 14
day: 14
part: frontier
minutes: 100
...
A language model does exactly one thing: given some text, guess what comes
next.

That's the whole job description. Everything you have seen one do — answering,
translating, summarising, rewriting, declining — comes out of that single
operation, made very large and then shaped. That sounds like a simplification
made for teaching. It isn't; by the end of today you'll have built each piece.

## Tokens

<Says who="wren">
It is worth pausing on how strange this is before we take it apart. Everything a
language model appears to know — history, arithmetic, the shape of an apology —
it picked up as a side effect of getting better at guessing the next chunk of
text. Nobody sat down and taught it any of those things directly.
</Says>

Models don't see letters, and they don't see words. They see **tokens** —
chunks of text, some of them whole words, some of them fragments, worked out
ahead of time by a procedure that looks for common pairs.

We can build that procedure. It's shorter than you'd expect.
...
the r's is structurally hard for it, in the way that counting the
...
That's a language model. A terrible one — its context is a single word —
...
Now the unexpected part. Put a few examples in the prompt, and the
...
behaviour is learnable: output format, refusal style, domain tone,
...
frequent pairs, later ones capture increasingly rare ones, and the
...
people once it is out in the world.
